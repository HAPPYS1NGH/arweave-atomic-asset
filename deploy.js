...
const ANT = "GGyMeoYjXCwKUFzGtBFquauYbCzhaec_k2VpQwAS_tU"
const SUBDOMAIN = "happysingh"

const jwk = JSON.parse(
  Buffer.from(process.env.WALLET64, "base64").toString("utf-8")
)
...
execSync(
  `cd ./dist && ardrive upload-file -s '${process.env.SEED}' -l ./ -F ${process.env.FOLDER_EID} --turbo`,
  _
)

console.log("Creating manifest...")
execSync(
  `ardrive create-manifest -s '${process.env.SEED}' -f ${process.env.FOLDER_EID} --turbo --dry-run > manifest.json`,
  _
)
...
execSync(
  `cd ./dist && ardrive upload-file -s "${process.env.SEED}" -l ./manifest.json --content-type application/x.arweave-manifest+json -F ${process.env.FOLDER_EID} --turbo > ../out.json`,
  _
)
